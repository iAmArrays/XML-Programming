	...
	<xsl:template match = "companyA | companyB">
		...
		<html>
			...
			<body>
				...
				<xsl:variable name="quantity" select="productName/@quantity | quantity" />
				...
				<h3>
					<xsl:value-of select='format-number(sum(//itemCustomer[productName/@quantity>5]/amount | //customer[quantity>5]/amountTotal), "$###,##0.00")'/> worth of goods currently on order
				</h3>
			</body>
		</html>
	</xsl:template>
